...
function SettingsModal({ open, onClose }) {
  ...
  const exportCSV = () => {
    ...
    const expenses = store.state.expenses.filter(e => e.date.startsWith(ym));
    const rows = [
      ['날짜', '내용', '카테고리', '카드', '금액'],
      ...expenses.sort((a,b)=>a.date.localeCompare(b.date))
        .map(e => [e.date, e.title, e.cat, e.card, e.amount])
    ];
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g,'""')}"`).join(',')).join('\n');
    const blob = new Blob(['\uFEFF'+csv], {type:'text/csv;charset=utf-8;'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `gravo-${ym}-소비내역.csv`;
    a.click(); URL.revokeObjectURL(url);
    toast(`${ym} 소비내역 CSV 저장됨`);
  };
  ...
  const generateReport = () => {
    ...
    const catTotals = expenses.reduce((a,e)=>{a[e.cat]=(a[e.cat]||0)+e.amount;return a;},{});
    const topCats = Object.entries(catTotals).sort((a,b)=>b[1]-a[1]).slice(0,5);
    const woori = store.state.accounts.find(a=>a.id==='woori');
    const fixed = store.state.fixed.reduce((s,f)=>s+f.amount,0);
    const report = `📊 Gravo 재정 월간 보고서 (${ym})\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `💳 이달 소비: ₩${totalExp.toLocaleString()} (${expenses.length}건)\n` +
      `🏦 우리은행 마이너스: ₩${(woori?.balance||0).toLocaleString()}\n` +
      `📌 고정비: ₩${fixed.toLocaleString()}\n\n` +
      `📂 카테고리별 소비\n` +
      topCats.map(([k,v])=>`  ${k}: ₩${v.toLocaleString()}`).join('\n') + '\n\n' +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `Generated by Gravo Finance`;
    const blob = new Blob([report], {type:'text/plain;charset=utf-8;'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `gravo-${ym}-보고서.txt`;
    a.click(); URL.revokeObjectURL(url);
    toast(`${ym} 보고서 저장됨`);
  };
  if (importPreview) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="데이터 관리">
      ...
      <div style={{display:'flex', flexDirection:'column', gap:8, marginBottom:18}}>
        ...
        <button className="btn" onClick={exportCSV} style={{justifyContent:'flex-start'}}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{marginRight:8}}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M8 13h2m0 0h4m-4 0v4"/></svg>
          이달 소비내역 CSV 내보내기
        </button>
        <button className="btn" onClick={generateReport} style={{justifyContent:'flex-start'}}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{marginRight:8}}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M16 13H8m8 4H8"/></svg>
          이달 월간 보고서 저장
        </button>
